...
exec xgraph -fg orange -bg green -bb -tk -x time -t "TCPRenoCWND" WindowVsTimeRenoOne &
exec xgraph -fg orange -bg green -bb -tk -x time -t "TCPRenoCWND" WindowVsTimeRenoAll &
...
set encoding utf8
set term pngcairo font "Impact,9"
```
...
set xlabel "t[s]" font "Impact, 10"
set ylabel "CWND [pkt]" font "Impact, 10"

# построение графика, используя значения
# 1-го и 2-го столбцов файла WindowVsTimeRenoOne
plot "WindowVsTimeRenoOne" using ($1):($2) with lines linecolor rgb "green" title "Размер окна TCP"
```
...
plot "WindowVsTimeRenoAll" using ($1):($2) with lines linecolor rgb "green" title "Размер окна TCP"
```
...
# подписи осей графика
set xlabel "t[s]" font "Impact, 10"
set ylabel "Queue Length [pkt]" font "Impact, 10"

# построение графика, используя значения
# 1-го и 2-го столбцов файла temp.q
plot "temp.q" using ($1):($2) with lines linecolor rgb "green" title "Текущая длина очереди"
```
...
# подписи осей графика
set xlabel "t[s]" font "Impact, 10"
set ylabel "Queue Avg Length [pkt]" font "Impact, 10"

# построение графика, используя значения
# 1-го и 2-го столбцов файла temp.a
plot "temp.a" using ($1):($2) with lines linecolor rgb "green" title "Средняя длина очереди"
```
